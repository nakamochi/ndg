...
fn processLndReportError(self: *Daemon, err: anyerror) !void {
    ...
    switch (err) {
        ...
        error.FileNotFound => {
            // FileNotFound from sendLightningReport() is ambiguous:
            // - missing TLS cert: LND is still starting / regenerating certs
            // - missing macaroon: wallet may be uninitialized after reset
            //
            // walletstatus only needs TLS, not macaroon. So only continue to
            // walletstatus if the TLS cert exists.
            std.fs.cwd().access(Config.LND_TLSCERT_PATH, .{}) catch |access_err| {
                return switch (access_err) {
                    error.FileNotFound => comm.write(self.allocator, self.uiwriter, msg_starting),
                    else => access_err,
                };
            };
        },
    ...
    var client = lndhttp.Client.init(.{ .allocator = self.allocator, .tlscert_path = Config.LND_TLSCERT_PATH }) catch |err2| {
        return switch (err2) {
            error.FileNotFound => comm.write(self.allocator, self.uiwriter, msg_starting),
            else => err2,
        };
